// $Id: hoeingCycle.cxx,v 1.10 2012/12/07 14:21:51 peiffer Exp $

#include <iostream>

using namespace std;

#include "include/hoeingCycle.h"
#include "include/hoeingHists.h"

#include "SFrameAnalysis/include/SelectionModules.h"
#include "SFrameAnalysis/include/HypothesisHists.h"

ClassImp( hoeingCycle );

hoeingCycle::hoeingCycle()
   : AnalysisCycle() {
...
void hoeingCycle::BeginCycle() throw( SError ) 
{
...
void hoeingCycle::EndCycle() throw( SError ) 
{
...
void hoeingCycle::BeginInputData( const SInputData& id ) throw( SError ) 
{
  ...
  RegisterHistCollection( new hoeingHists("NoCuts") );
  RegisterHistCollection( new HypothesisHists("Chi2_NoCuts", m_chi2discr ) );

  //histograms with and without b tagging
  RegisterHistCollection( new hoeingHists("BTag") );
  RegisterHistCollection( new hoeingHists("NoBTag") );
  ...
  RegisterHistCollection( new hoeingHists("TopSel") );
...
void hoeingCycle::EndInputData( const SInputData& id ) throw( SError ) 
{
  AnalysisCycle::EndInputData( id );
}

void hoeingCycle::BeginInputFile( const SInputData& id ) throw( SError ) 
{
...
void hoeingCycle::ExecuteEvent( const SInputData& id, Double_t weight) throw( SError ) 
{
  ...
  // all thse BaseHists* vairables private member variables of hoeingCycle and
